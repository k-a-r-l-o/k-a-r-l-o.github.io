...
$sqlPositions = "SELECT position_name, position_slot, YearLvl FROM positions WHERE council_id = $council_id";
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    while ($rowPosition = $resultPositions->fetch_assoc()) {
        ...
        $positionYearLvl = $rowPosition['YearLvl'];

        if ($positionYearLvl != 'All Year' && $positionYearLvl != $yearLvl) {
            continue; // Skip positions not meant for this user's year level
        }
                        ...
                        <div class="card" data-position="<?php echo $positionName; ?>" data-slot="<?php echo $positionSlot; ?>">
                            ...
                            <div class="cardcontent">
                                <?php while ($rowCandidate = $resultCandidates->fetch_assoc()) : ?>
                                    ...
                                    $candidateImage = htmlspecialchars($rowCandidate['candPic'] ?? 'uploads/default.png');
                                    $partyListName = htmlspecialchars($rowCandidate['name_partylist'] ?? '');
                                    ?>
